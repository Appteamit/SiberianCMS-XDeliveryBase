...
class Xdelivery_Form_ShippingMethod_Free extends Siberian_Form_Abstract
{
    ...
    public function init() {
        ...
        $hl6 = p__("xdelivery", "Free Shipping");
        $helpText6 = '<div class="col-md-12"><div  class="alert alert-info">'.  $hl6 .'</div></div>';
        $this->addSimpleHtml("helper_text6", $helpText6);

        $method_type = $this->addSimpleHidden("method_type")->setValue('free');

        $this->addSimpleCheckbox('status', p__('xdelivery', 'Status'));
        
        /** label name */
        $label_name = $this->addSimpleText('label_name', p__('xdelivery', 'Label Name'))->setRequired(true)->setValue('Free Shipping');

        /** amount  */
        $amount = $this->addSimpleText('amount', p__('xdelivery', 'Minimum Amount'))->setRequired(false)->setValue(0);
